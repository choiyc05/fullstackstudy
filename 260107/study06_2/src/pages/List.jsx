...
const List = () => {
    ...
    useEffect (()=>{
        if (state === 1) setArrs(list.filter(v=>v.accept === state))
            else if (state === 2) setArrs(list.filter(v=> v.accept === state))
            else setArrs([...list])
        },[state]
    )
    ...
    return (
        <div className="container mt-3">
            ...
            <div className="list-group mt-2 text-center">
                {
                    arrs.map((v,i)=>
                    <button key={i} type="button" className={`list-group-item m-1 display-6 ${v.accept ===1 ? "list-group-item-success" : "list-group-item-warning"}`} 
                    onClick={()=>navigateDetail(i)}>{v.title}</button>)
                }
            </div>
        </div>
    )
